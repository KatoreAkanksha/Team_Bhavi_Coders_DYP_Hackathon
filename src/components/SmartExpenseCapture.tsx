...
export function SmartExpenseCapture({ onSubmit }: { onSubmit?: (data: ExpenseData) => void }) {
    ...
    reader.onload = (event) => {
      if (event.target?.result) {
        ...
        const processingTimeout = setTimeout(() => {
          ...
          // Convert USD to INR
          setAmount(usdToInr(65.99).toString());
  ...
  const handleCameraCapture = () => {
    if (isCameraActive) {
      ...
      setTimeout(() => {
        ...
        // Convert USD to INR
        setAmount(usdToInr(12.50).toString());
    ...
    // Toggle camera state after handling the current state
  ...
  // This effect is no longer needed as we're handling timeouts in their respective functions
  useEffect(() => {
